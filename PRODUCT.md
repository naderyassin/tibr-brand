...
Robabikia is one brand house with two distinct experiences. They share a name and an audience, not a treatment.
...
- **Store / app layer (everything else) — register: `product`.** A complete e-commerce application: the three category pages (perfumes, clothing, sneakers), product detail, cart, checkout, order confirmation, search, wishlist, account dashboard, admin control panel, and dedicated full-page login / sign-up / forgot-password. This layer is a **ground-up redesign on a brand-new premium design system** (see **Store Design System** below). All Egyptian-theme-derived styling is removed and replaced here. No modals for auth: every flow is its own full page.

The two surfaces must still read as one house. The through-line is a single refined gold accent and the Robabikia name, not shared ornament or shared type.
...
Robabikia is a luxury Egyptian brand selling perfumes, contemporary-classic clothing, and leather shoes. The name evokes the street vendors of old Cairo — a portal to collective memory. Every product is positioned as a handcrafted story, not just a purchase.
...
Success looks like: a visitor feeling the brand's soul on the landing, then moving into a store that feels equally considered, organized, and unmistakably high-end across all three categories.
...
- **Accent:** a single refined, desaturated gold, carried over from the brand as the through-line so the store still reads as Robabikia. Used with restraint to mark what matters (price, primary action, active state), never as decoration. One accent only; no second accent color competing across categories.
- **Unity across categories:** perfume, clothing, and sneakers share one layout grammar, one type system, one component set. The product photography differentiates the categories; the chrome does not.
