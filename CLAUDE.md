...
- **CSS:** the shared design system now lives inside the client at `client/src/styles/store/` — `client/src/styles/index.css` is just `@import "./store/store.css"` (and `AdminLayout.jsx` loads `store/admin.css`). Do not duplicate store styles elsewhere; edit `client/src/styles/store/`.
...
- **Backend:** a modular Express app under `server/` — `server/index.js` (app wiring, static serving, SPA fallback, `listen`), `server/config.js` (env), `server/db.js` (Supabase client factories), `server/middleware/auth.js` (requireUser/Admin/SuperAdmin), `server/services/{products,discounts,orders}.js` (business logic), `server/routes/*.js` (one Express Router per domain, full `/api` paths), `server/lib/parse.js` (shared input helpers), `server/paymob.js`. Wired to the client via `lib/api.js` (products, profile, addresses, orders, checkout, admin). Cart persists client-side via the Zustand store.
- **Bilingual pattern:** the lang store (`client/src/stores/lang.js`) drives `<html lang/dir>`; components render AR/EN inline.
- **Dev:** `npm run dev:all` (Express on :3000 + Vite on :5173, which proxies `/api` and `/assets` to Express). **Build:** `npm run client:build` → `dist/client/`; the `server/` app serves that build for store routes. If `dist/client` is missing, store routes are unavailable until a build is run.
