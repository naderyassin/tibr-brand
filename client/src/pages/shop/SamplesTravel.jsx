...
const meta = COLLECTIONS.samples;

export default function SamplesTravel() {
  const { sub } = useParams();
  const subLabel = sub ? getSubLabel(meta.subs, sub) : null;
  return (
    <CollectionPage
      title={subLabel || meta.title}
      breadcrumb={subLabel ? `Sample & Travel Size / ${subLabel.split(" —")[0]}` : meta.breadcrumb}
      sortDefault={meta.sortDefault}
      sub={sub || null}
      subField={meta.subField}
      predicate={(p) => p.listing_type === "sample"}
    />
  );
}
